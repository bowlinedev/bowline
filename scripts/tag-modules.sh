...
echo "created tags $version, cmd/bowline/$version, transport/websocket/$version, mcp/$version, agent/$version, and playground/$version; push with: git push origin $version cmd/bowline/$version transport/websocket/$version mcp/$version agent/$version playground/$version"
