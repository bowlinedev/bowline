...
git tag -a -m "bowline sql idempotency store $version" "stores/sql/$version" "$commit"
git tag -a -m "bowline redis idempotency store $version" "stores/redis/$version" "$commit"
...
echo "  git push origin cmd/bowline/$version transport/websocket/$version mcp/$version agent/$version playground/$version contracttest/$version adapters/fiber/$version gateway/$version registry/$version otel/$version stores/sql/$version stores/redis/$version"
