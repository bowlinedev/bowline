...
`@bowline/vue` gives every query a `useQuery` composable with `data`, `error`, and `loading` refs that re-run when the input changes, and `queryOptions` shaped for `@tanstack/vue-query` when the app already uses it.
...
Proof: `pnpm --filter @bowline/vue test` covers both paths, including the resolved TanStack key `[["users", "get"], { id: 1 }]`.
