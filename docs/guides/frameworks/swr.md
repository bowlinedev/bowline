...
`@bowlinedev/swr` turns every query into a `useQuery` hook over `useSWR` and every mutation into a `useMutation` hook over `useSWRMutation`, with the shared key shape.
...
Proof: `pnpm --filter @bowlinedev/swr test` renders both hooks against a fake client and checks the cache key.
