...
`@bowline/svelte` gives every query a readable store with `loading`, `data`, `error`, and `refresh()`, and every mutation a `mutate` function with a `pending` store.
...
Proof: `pnpm --filter @bowline/svelte test`; the SvelteKit example under `examples/sveltekit` uses the stores in `+page.svelte`, see `sveltekit.md`.
