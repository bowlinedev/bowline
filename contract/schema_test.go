...
func TestSchemaShape(t *testing.T) {
	...
	if s.ID != "https://bowline.dev/spec/contract/1.0/contract.schema.json" {
		t.Fatalf("unexpected $id %q", s.ID)
	}
	for _, name := range []string{"TypeDecl", "Type", "Field", "Rule", "EnumValue", "ErrorDecl", "Procedure", "Position"} {
